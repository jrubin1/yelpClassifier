...
class YelpClassifier(object):
    ...
    def calculate_likelihoods(self, train_rdd):
        ...
        class_likelihoods = train_rdd \
                            .flatMap(YelpClassifier.review_to_word_counts)\
                            .reduceByKey(YelpClassifier.add_review_counts)\
    ...
    def calculate_num_reviews_and_words_per_num_stars(self, train_rdd):
        ...
        num_reviews_words_per_num_stars = train_rdd \
                                          .map(YelpClassifier.review_to_num_stars_num_words)\
                                          .reduceByKey(YelpClassifier.add_review_and_word_counts)
                                            # ._____(_____) \
                                            # ._____(_____) 
    ...
    def classify_reviews(self, test_rdd):
        ...
        predictions = test_rdd \
                      .flatMap(YelpClassifier.review_to_num_stars_and_word_pairs)\
                      .map(self.words_to_log_likelihoods)\
                      .reduceByKey(YelpClassifier.add_log_likelihoods)\
                      .map(self.likelihood_to_posterior)\
                      .map(self.likelihood_to_posterior)\
                      .reduceByKey(YelpClassifier.find_max_posterior)\
                      .map(YelpClassifier.review_id_only_as_key)\
                      .sortByKey()

                      #could be bug above at map(words_to_log_likelihoods). could be reduce instead or somethign like that
	...
	print(review)
	result = []
        for word in review[2].split(" "):
	    print((review[1], word), 1)
            result.append(((review[1], word), 1))
        return result
        raise NotImplementedError()
    ...
    def counts_to_probabilities(self, num_stars_and_word_counts):
        ...
        print('printing counts to probabilities')
	print(num_stars_and_word_counts[0][0])
	print(num_stars_and_word_counts)
	wordAndProbDict = {}
        wordAndProbDict[num_stars_and_word_counts[0][1]] = self.compute_likelihood(num_stars_and_word_counts[1], float(num_stars_and_word_counts[0][0]))
        return num_stars_and_word_counts[0][0], wordAndProbDict
        raise NotImplementedError()
    ...
    def review_to_num_stars_num_words(review):
        ...
        print('printing review in review to num stars num words')
	print(review)
	count = 0
        for word in review[2].split(" "):
            count += 1
	print(review[1], (1, count))
        return review[1], (1, count)
        raise NotImplementedError()
    ...
    def find_max_posterior(num_stars_posterior1, num_stars_posterior2):
        ...
        if num_stars_posterior1[1][1] > num_stars_posterior2[1][1]:
            return num_stars_posterior1[1][0], num_stars_posterior1[1][1]
        else:
            return num_stars_posterior2[1][0], num_stars_posterior2[1][1]
        raise NotImplementedError()
